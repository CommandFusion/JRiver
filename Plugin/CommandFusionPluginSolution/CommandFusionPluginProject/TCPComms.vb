...
Imports System.Threading
    ...
    Public Class TCPComms
        ...
        Private m_Port As Integer = 8080
        ...
        Public Event DataReceived(ByVal IPAddress As String, ByVal MsgBytes As Byte())
            ...
            While True
                ...
                Catch ex As Exception
                    If Not TypeOf ex Is ThreadAbortException Then
                        GlobalHelpers.Log("WaitingForClient Error: " & ex.ToString)
                    End If
                End Try
            End While
        ...
        Public Sub SendMsg(ByVal msg As String)
            ...
            m_SendQueue.Add(0)
            m_SendQueue.Add(0)
            GlobalHelpers.Log("Sending " & msg)
            Debug.WriteLine("Sending: " & msg)
        End Sub

        Public Sub SendMsg(ByVal msg As String, ByVal remoteIP As String, ByVal port As Integer)
            m_SendQueue.Add(msg)
            m_SendQueue.Add(remoteIP)
            ...
            While True
                ...
                Else
                    If m_SendQueue.Count > 0 And Not m_PauseSending Then
                        ...
                        Dim remoteIP As String = m_SendQueue(0)
                        m_SendQueue.RemoveAt(0)
                        ...
                        Try
                            ...
                            For i As Integer = 0 To TCPmsg.Length - 1
                                If i < (TCPmsg.Length - 3) AndAlso TCPmsg(i) = "\" AndAlso TCPmsg(i + 1) = "x" Then
                                ...
                                Else
                                    ...
                                    Catch ex As Exception
                                        ' Skipped a strange character, such as chinese symbol
                                    End Try
                                End If
                            Next
                        ...
                        Catch ex As Exception
                            If Not TypeOf ex Is ThreadAbortException Then
                                GlobalHelpers.Log("Send Error: " & ex.ToString)
                            End If
                        End Try
                    Else
                        Thread.Sleep(10)
        ...
        Public Sub ReadSocket()
            ...
            While True
                Try
                    If s.Connected Then
                        ...
                        If ret > 0 Then
                            ...
                            RaiseEvent DataReceived(clientIP, receive)
                ...
                Catch ex As Exception
                    If Not TypeOf ex Is ThreadAbortException Then
                        GlobalHelpers.Log("ReadSocket Error: " & ex.ToString)
                        Exit While
                    End If
                End Try
            End While
